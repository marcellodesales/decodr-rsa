...
import static com.signr.rsa.core.Rsa.LOG_ARROW;
...
/**
 * The Rsa Public Key implements the public key of the RSA algorithm. It contains the public keys N
 * and E.
 * 
...
public final class RsaPublicKey {
   ...
   */
  private RsaPublicKey(RsaFactors givenFactors) {
    this.factors = givenFactors;

    // What was observed was that when the prime numbers are small, the reverse function fails...
    // The new primes must be big enough to make N a big number.
    this.n = this.factors.getP() * this.factors.getQ();

    log.add(LOG_ARROW + "Calculating public keys");
    log.add("          N = P * Q; N = " + DECIMAL_FORMATTER.format(this.n));

    // FI = (P-1) * (Q-1);
    this.fi = Algebra.SINGLETON.getEuler(this.factors.getP(), this.factors.getQ());
    this.e = this.generateKeyE();

    log.add("          E = " + DECIMAL_FORMATTER.format(this.e));
    log.add("");

    log.add("       Public Key (N,E) = (" + DECIMAL_FORMATTER.format(this.n) + " , "
  ...
  /**
   * Factory method used on the RSAFactors instance, which include the prime numbers P and Q.
   * Constructing the object using this method gives access to all the utility variables and
   * execution log.
   * 
  ...
  /**
   * Factory method based on the key N and the key E. Constructing the object with this constructor
   * DOES NOT produce the execution logs and other utility variables.
   * 
   * @param keyN
   * @param keyE
  ...
  private long generateKeyE() {
    ...
    while (mdc != 1) {
      ...
      this.log.add("           MCD(" + DECIMAL_FORMATTER.format(aux) + " , "
          + DECIMAL_FORMATTER.format(fi) + ") = " + DECIMAL_FORMATTER.format(mdc));
      aux++;
      mdc = Algebra.SINGLETON.getMDC(aux, this.fi);
    }
    this.log.add("           MCD(" + DECIMAL_FORMATTER.format(aux) + " , "
        + DECIMAL_FORMATTER.format(fi) + ") = " + DECIMAL_FORMATTER.format(mdc) + " Correct!");
    return aux;
  }
   ...
   * @throws IllegalStateException in case the public key object was constructed with the values for
   *         N and E, without passing over the generation process.
   */
  public long getPrimeNumberP() {
    if (this.factors != null) {
      ...
      throw new IllegalStateException(
          "The prime number 'P' is unknown: public keys not generated here.");
    }
  }
  ...
  public long getPrimeNumberQ() {
    if (this.factors != null) {
      ...
      throw new IllegalStateException(
          "The prime number 'Q' is unknown: public keys not generated here.");
    }
  }
  ...
  public String[] getLogEntries() {
    List<String> logEntries =
        new ArrayList<>(this.factors.getLogEntries().length + this.log.size());
